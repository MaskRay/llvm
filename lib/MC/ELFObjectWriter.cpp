...
void ELFObjectWriter::executePostLayoutBinding(MCAssembler &Asm,
  ...
  for (const std::pair<StringRef, const MCSymbol *> &P : Asm.Symvers) {
    ...
    // version definitions, 2) consistently suppress the original symbol. GNU as
    // keeps the original symbol in the symbol table for defined @ and @@, but
    // suppresses it for other cases (@@@ or undefined). The original symbol is
